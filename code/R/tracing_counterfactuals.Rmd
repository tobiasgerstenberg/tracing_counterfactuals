...
author: "Jonathan Kominsky, Tobias Gerstenberg & Henrik Singmann"
...
library("TreeBUGS")     # for estimating the hierachical Bayesian MPT model
                        # TreeBUGS version 1.4.4 has a bug, but prior versions work.
...
ggsave("../../figures/plots/experiment1a_bars.pdf",
       width = 10,
       height = 6)
```
...
We also can investigate peaks and 80% HDIs in tabular form:
...
pars %>% 
  ...
  mode_hdcih(estimate, .width = c(0.80))
```
...
tmp_diff %>% 
  group_by(inter) %>% 
  mean_hdcih(estimate, .width = c(0.80, 0.95))
```
...
tmp_diff %>%   
  ...
  geom_halfeyeh(.width = c(0.80, 0.95), point_interval = mode_hdcih) +
...
sigmas %>%
  group_by(parameter) %>%
  mode_hdcih(.value, .width = c(0.80, 0.95))
```
...
ggsave("../../figures/plots/experiment2_bars.pdf",
       width = 8,
       height = 6)
```
...
ggsave("../../figures/plots/experiment2_estimates.pdf",
       width = 4,
       height = 8)
```
...
ggsave("../../figures/plots/experiment3_estimate.pdf",
       width = 4,
       height = 4)
```
